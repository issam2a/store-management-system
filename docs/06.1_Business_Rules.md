...
COGS shall be calculated using the cost snapshot stored on each completed sale item.

When a sale is completed, the system shall capture the product's current purchase cost and store it as the sale item's unit cost.

Historical sale costs shall remain unchanged even if the product's current purchase cost changes later.

**Related:** BR-062, BR-070

---

## BRL-080A — Sale Cost Snapshot

When a sale is completed, the system shall store the product's current purchase cost as the sale item's unit cost.

This value shall be preserved as historical transaction data and used for profitability reporting.
...
## BRL-080B — Gross Margin When Revenue Is Zero

BRL-080B: Gross Margin shall be calculated as Gross Profit ÷ Revenue × 100 for completed sales when Revenue is greater than zero. When Revenue equals zero, Gross Margin shall be represented as None (undefined), while Gross Profit shall still be calculated as Revenue − COGS. Therefore, a completed sale with zero revenue and positive COGS shall produce a negative Gross Profit and an undefined Gross Margin rather than a zero Gross Margin.
...
## OD-002 — Future Inventory Costing Enhancement

V1 uses a cost snapshot methodology.

The exact inventory valuation methodology
